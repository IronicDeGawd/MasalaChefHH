...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    const startButton = this.add
      .rectangle(width / 2, height - 160, 280, 70, 0x8b4513)
      .setInteractive()
      .on("pointerdown", () => {
    ...
    // Start button text
    this.add
      .text(width / 2, height - 160, "Start Cooking!", {
        font: "32px Arial",
        fill: "#FFFFFF",
      })
      .setOrigin(0.5);
  ...
  createEnvironmentOptions(width, yPosition) {
    ...
    environments.forEach((env) => {
      ...
      const button = this.add
        .rectangle(env.x, yPosition, 260, 50, color)
        .setOrigin(0.5);
      ...
      if (env.id === this.selectedEnvironment) {
        this.add
          .rectangle(env.x, yPosition, 280, 70, 0xffd700)
          .setOrigin(0.5)
          .setDepth(-1);
      }
    });
  ...
  createRecipeOptions(width, yPosition) {
    ...
    recipes.forEach((recipe) => {
      ...
      const button = this.add
        .rectangle(recipe.x, yPosition, 260, 50, color)
        .setOrigin(0.5);
      ...
      if (recipe.id === this.selectedRecipe) {
        this.add
          .rectangle(recipe.x, yPosition, 280, 70, 0xffd700)
          .setOrigin(0.5)
          .setDepth(-1);
      }
    });
  ...
  highlightSelected(items, selectedId, yPosition) {
    ...
    this.add
      .rectangle(selectedItem.x, yPosition, 280, 70, 0xffd700)
      .setOrigin(0.5)
      .setDepth(-1);
  }
